function OutputSignal = FFTFixedv3(InputSignal,bits)
% FFTFIXED Fixed-Point fft transform;  Increase the integer bits after each stage with no truncation error;
...
Stages = log2(N);
temp = fi(S,1,bits+Stages,bits-1);
if FFTn~=2^p
...
%                 b=bitsra(s(pos)-fi(s(pos+Half).*w,1,bits,bits-1),1);             % 2-nd part of the "butterfly" creating operation
                bit_pre = bits+stage-1;
                bit_frac = bits-1;
                bit_new = bits+stage;
                a=fi(s(pos,:)+fi(fi(real(s(pos+Half,:)).*real(w),1,bit_pre,bits-1)-fi(imag(s(pos+Half,:)).*imag(w),1,bit_pre,bit_frac)+...
                    (fi(imag(s(pos+Half,:)).*real(w),1,bit_pre,bits-1)+fi(real(s(pos+Half,:)).*imag(w),1,bit_pre,bits-1))*1j,1,bit_pre,bit_frac),1,bit_new,bit_frac); 
                b=fi(s(pos,:)-fi(fi(real(s(pos+Half,:)).*real(w),1,bit_pre,bit_frac )-fi(imag(s(pos+Half,:)).*imag(w),1,bit_pre,bit_frac)+...
                    (fi(imag(s(pos+Half,:)).*real(w),1,bit_pre,bit_frac )+fi(real(s(pos+Half,:)).*imag(w),1,bit_pre,bits-1))*1j,1,bit_pre,bit_frac),1,bit_new,bit_frac);
